...
# Add directories for evm stuff, exit if an error occurs
if ! mkdir -p $EVM_HOME_DIR 2>/dev/null; then
    echo "Failed to create evm folder. Aborting..."
    exit 1
fi
...
sed -i -e 's/"hrpIBCRecords": \[\]/"hrpIBCRecords": \[{"hrp": "stars", "nft_source_channel": "channel-0"}\]/g' $CHAIN_DIR/config/genesis.json
...
cd ../Gravity-Bridge/solidity
npm run evm > $EVM_LOG_FILE_PATH 2>&1 &
cd ../../gravity-bridge-scripts

sleep 3

#./contract-deployer.sh
#cd ..

echo "-------- EVM started and contracts deployed! --------"

echo "Setting up orchestrator..."
../Gravity-Bridge/orchestrator/target/debug/gbt --home $GBT_DIR init
../Gravity-Bridge/orchestrator/target/debug/gbt --home $GBT_DIR keys set-ethereum-key --key $VALIDATOR_ETH_PRIVATE_KEY
../Gravity-Bridge/orchestrator/target/debug/gbt --home $GBT_DIR keys set-orchestrator-key --phrase "$ORCHESTRATOR_MNEMONIC"
#../Gravity-Bridge/orchestrator/target/debug/gbt --home $GBT_DIR keys register-orchestrator-address --validator-phrase "$VALIDATOR_MNEMONIC" --fees 0stake

# If you get troubles on this line, you might be using mac. In that case: https://ben.lobaugh.net/blog/205337/how-to-fix-the-sed-command-expects-error-on-mac-os
gsed -i -e '/\[orchestrator\]/a check_eth_rpc = false' $GBT_DIR/config.toml

echo "Starting orchestrator..."
echo "Creating log file at $GBT_LOG_FILE_PATH"
../Gravity-Bridge/orchestrator/target/debug/gbt --home $GBT_DIR orchestrator --fees 0stake > $GBT_LOG_FILE_PATH 2>&1 &

sleep 5
echo "-------- Orchestrator started! --------"

echo "Deploying ICS721 to Stargaze"
$STARGAZE_BINARY tx wasm store ./cw-nfts/artifacts/cw721_base-aarch64.wasm --node http://:$STARGAZE_RPC_PORT --home $STARGAZE_CHAIN_DIR --keyring-backend test --from kaare -y --fees 500000stake --gas 20000000 --chain-id $STARGAZE_CHAIN_ID -b block
$STARGAZE_BINARY tx wasm store ./ics721/artifacts/ics721_base-aarch64.wasm --node http://:$STARGAZE_RPC_PORT --home $STARGAZE_CHAIN_DIR --keyring-backend test --from kaare -y --fees 500000stake --gas 20000000 --chain-id $STARGAZE_CHAIN_ID -b block
...
cp rly-config.yaml $RELAYER_HOME_DIR/config/config.yaml
